...
use crate::game::music::{{Release, MarketingCampaignType, ReleaseType, ActiveMarketingCampaign}};
...
impl TerminalUI {{
    ...
    pub fn display_game_state(&mut self, game: &Game) -> Result<(), Box<dyn std::error::Error>> {{
        ...
        // Display new RELEASES & MARKETING section
        ...
        for r_ref in game.just_released_music.iter().chain(game.band.singles_released.iter()).chain(game.band.albums_released.iter()) {{
            all_releases.push(r_ref);
        }}
        all_releases.sort_by(|a, b| b.week_released.cmp(&a.week_released)); // Sort descending
        all_releases.dedup_by_key(|r| r.id);

        for (i, release) in all_releases.iter().take(5).enumerate() {{
            let type_str = if release.release_type == ReleaseType::Album {{ "Album" }} else {{ "Single" }};
            let marketing_info = if !release.active_marketing.is_empty() {{
                format!("(Marketing: {{:?}} - {{}} weeks left)", 
                    release.active_marketing[0].campaign_type, 
                    release.active_marketing[0].end_week.saturating_sub(game.week)
                )
            }} else {{ "".to_string() }};
            execute!(self.stdout, Print(&format!(
                "  {{}}. {{}} ({{}}) - Q:{{}}/100, Sales:{{}}, Income:{{}} {{}}\n",
                i + 1, release.name, type_str, release.release_quality, 
                release.initial_sales_score, format_money(release.total_income_generated as i32),
                marketing_info
            )))?;
        }}
    ...
    fn wait_for_key(&mut self) -> Result<(), Box<dyn std::error::Error>> {{
        loop {{ if let Event::Key(_) = event::read()? {{ break; }} }}
        Ok(())
    }}
    ...
    fn select_release_for_marketing(&mut self, game: &Game) -> Result<Option<u32>, Box<dyn std::error::Error>> {{
        loop {{
            ...
            for (idx, release) in available_releases.iter().enumerate() {{
                let type_str = if release.release_type == ReleaseType::Album {{ "Album" }} else {{ "Single" }};
                execute!(self.stdout, Print(&format!("[{}] {{}} ({{}}) - Q:{{}}/100\n", idx, release.name, type_str, release.release_quality)))?;
            }}
    ...
    fn display_active_campaigns(&mut self, game: &Game) -> Result<(), Box<dyn std::error::Error>> {{
        ...
        let all_releases = game.just_released_music.iter()
            .chain(game.band.singles_released.iter())
            .chain(game.band.albums_released.iter());

        for release in all_releases {{
            if !release.active_marketing.is_empty() {{
                found_any = true;
                execute!(self.stdout, Print(&format!("\nRelease: {{}} (Q:{{}}/100)\n", release.name, release.release_quality)))?;
                for campaign in &release.active_marketing {{
